...
constexpr float PEEK_RIGHT_AMT = -0.2;  // amount to peek right
#else
constexpr float KP_ROT = 1.0;  // proportional gain for drivetrain 
constexpr float KI_ROT = 0;      // integral gain for velocity
constexpr float KD_ROT = 0;      // derivative gain for drivetrain
constexpr float MAX_ISUM_ROT = 0;  // maximum integral sum term for drivetrain
constexpr float MAX_OUTPUT_ROT = 2.0;  // maximum output for drivetrain rotation controller

constexpr float ERR_DEADZONE_ROT = 0.05;  // error deadzone for drivetrain rad
...
constexpr tap::algorithms::SmoothPidConfig drivetrainPIDConfig{KP_ROT, KI_ROT, KD_ROT, MAX_ISUM_ROT, MAX_OUTPUT_ROT, 1.0f, 0.0f, 1.0f, 0.0f, ERR_DEADZONE_ROT};
